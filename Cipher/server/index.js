...
// Redirect cipherprivate.com.au to cipherprivate.com (301 permanent, SEO-safe)
app.use((req, res, next) => {
  const host = req.headers.host || '';
  if (host.includes('cipherprivate.com.au')) {
    const target = 'https://cipherprivate.com' + req.originalUrl;
...
// PUBLIC ROUTES — no auth, always accessible (Google crawler, health checks)
...
.btn{width:100%;padding:14px;background:#c9a96e;color:#080808;border:none;font-family:'Montserrat',sans-serif;font-size:9px;letter-spacing:4px;text-transform:uppercase;font-weight:700;cursor:pointer;margin-top:6px}
    ...
    .then(function(data) {
      ...
      if (data.expired || data.alreadyUsed || data.error) {
        ...
        document.getElementById('expiredMsg').textContent = data.alreadyUsed
          ? 'This link has already been used.' : data.expired
          ? 'This link has expired. Please request a new one.' : data.error || 'Invalid link.';
        return;
      }
      ...
      document.getElementById('docMeta').textContent = 'Shared by ' + data.senderName + '  |  Expires ' + exp;
...
function nxt(el,n){ if(el.value.length>1) el.value=el.value.slice(-1); if(el.value&&n<=5) document.getElementById('d'+n).focus(); }
...
function verify(){
  ...
  fetch('/api/otp/verify/'+TOKEN,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({otp:otp})})
    .then(function(res){
      if(res.ok){
        ...
        return res.blob().then(function(blob){
          ...
          a.href=url; a.download=document.getElementById('docName').textContent; a.click();
          URL.revokeObjectURL(url);
        });
      } else {
        return res.json().then(function(d){ showErr(d.error||'Verification failed.'); btn.disabled=false; btn.textContent='Verify & Access Document'; });
      }
    })
    .catch(function(){ showErr('Connection error. Please try again.'); btn.disabled=false; btn.textContent='Verify & Access Document'; });
}
...
// Serve index.html — MUST return 200 for all non-API routes including Google crawler
const possiblePaths = [
...
app.get('*', function(_req, res) {
  if (fs.existsSync(clientHtml)) {
    // Always return 200 — never 401 — for the public website
