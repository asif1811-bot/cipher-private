...
app.get('/api/health', (_req, res) => res.json({ status: 'operational', service: 'Cipher Private', timestamp: new Date().toISOString() }));

app.get('*', (_req, res) => {
  const htmlPath = path.join(__dirname, '../../index.html');
  if (fs.existsSync(htmlPath)) res.sendFile(htmlPath);
  else res.send('<h1 style="font-family:serif;text-align:center;margin-top:20vh;color:#c9a96e">Cipher Private</h1>');
});
